...
def analyze_simulation_results(results_df: pd.DataFrame):
    ...
    phase_analysis = results_df.groupby(['Phase', 'Score_Context']).agg({
        'Expected_Impact': ['mean', 'count'],
        'Leverage': 'mean'
    }).round(4)
    phase_analysis.columns = ['Impact', 'Samples', 'Avg_Leverage']
    ...
    # Create pivot table for clear visualization with fixed order
    pivot = matrix_df.pivot(index='Phase', columns='Score', values='Impact')
    # Reindex to enforce fixed order
    pivot = pivot.reindex(index=PHASES, columns=SCORE_CONTEXTS)
    
    # Add color styling - continuous gradient (red-white-green)
    def color_bar(val, vmin, vmean, vmax):
        if pd.isna(val):
            return ''
        # Normalize value to [-1, 1] where 0 is mean
        if val >= vmean:
            # Green side
            norm = (val - vmean) / (vmax - vmean) if vmax > vmean else 0
            norm = min(max(norm, 0), 1)
            # Interpolate white to green
            r = int(255 * (1 - norm) + 40 * norm)   # 255 to 40
            g = int(255 * (1 - norm) + 167 * norm)  # 255 to 167
            b = int(255 * (1 - norm) + 69 * norm)   # 255 to 69
        else:
            # Red side
            norm = (vmean - val) / (vmean - vmin) if vmean > vmin else 0
            norm = min(max(norm, 0), 1)
            # Interpolate white to red
            r = int(255 * (1 - norm) + 220 * norm)  # 255 to 220
            g = int(255 * (1 - norm) + 53 * norm)   # 255 to 53
            b = int(255 * (1 - norm) + 69 * norm)   # 255 to 69
        return f'background-color: rgb({r},{g},{b})'

    vmin = float(pivot.min().min())
    vmax = float(pivot.max().max())
    vmean = float(pivot.mean().mean())

    def color_gradient(val):
        return color_bar(val, vmin, vmean, vmax)

    # Display matrix with gradient colors only, no numbers
    styled_pivot = pivot.style.applymap(color_gradient).format("")
